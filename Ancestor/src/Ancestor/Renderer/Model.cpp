...
namespace Ancestor {
	...
	void Model::AddTexture(Ref<Texture3D> texture)
	{
	...
	void Model::loadModel(const std::string& path)
	{
		...
		{
			AC_CORE_ERROR(importer.GetErrorString());
			return;
		}
	...
	Ref<Mesh> Model::processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		// 3. normal maps
		std::vector<Ref<Texture>> normalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal");
		textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
		// 4. height maps
		std::vector<Ref<Texture>> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height");
	...
	std::vector<Ref<Texture>> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				Ref<Texture> texture;
				texture = Texture3D::Create(str.C_Str());
